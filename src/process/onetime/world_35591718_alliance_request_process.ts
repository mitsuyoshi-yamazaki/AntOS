...
export interface World35591718AllianceRequestProcessState extends ProcessState {
  /** parent room name */
  readonly p: RoomName
...
// Game.io("launch -l World35591718AllianceRequestProcess room_name=W19N6")
export class World35591718AllianceRequestProcess implements Process, Procedural {
  ...
  public encode(): World35591718AllianceRequestProcessState {
    return {
      t: "World35591718AllianceRequestProcess",
      l: this.launchTime,
      i: this.processId,
  ...
  public static decode(state: World35591718AllianceRequestProcessState): World35591718AllianceRequestProcess {
    return new World35591718AllianceRequestProcess(state.l, state.i, state.p, state.targetRoomNames, state.lastSpawnTime)
  }

  public static create(processId: ProcessId, parentRoomName: RoomName): World35591718AllianceRequestProcess {
    return new World35591718AllianceRequestProcess(Game.time, processId, parentRoomName, null, 0)
  }
  ...
  private getTargetRooms(): RoomName[] {
    ...
    for (let i = 0; i < roomCount; i += 1) {
      ...
      if (route === ERR_NO_PATH) {
        PrimitiveLogger.fatal(`World35591718AllianceRequestProcess.getTargetRooms() parent: ${roomLink(parentRoomName)}, no path from ${roomLink(roomName)} to ${roomLink(closestRoom.roomName)}`)
        return result
      }
